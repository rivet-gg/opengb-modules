...
export function getLobbyConfig(userConfig: Config, lobbyTags: Record<string, string>): LobbyConfig {
	let config = userConfig.lobbies;

    // Apply rules
    for (const rule of this.config.lobbyRules) {
 ...
 */
function lobbyTagsMatch(query: Record<string, string>, target: Record<string, string>): boolean {
